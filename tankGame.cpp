...
GLfloat TranslationTank[NumAxes] = {0.0, 0.0, 0.0};
GLfloat RotationTank[NumAxes] = {0.0, 0.0, 0.0};
...
init()
{
    ...
    //copy and paste for the multiple pictures, rename
...
//---------D R A W   S H A P E S   F U N C T I O N S----------------
void
drawTankCube(vec4 ka, vec4 kd, vec4 ks, vec3 translation, vec3 scalation, mat4 transformTank)
{
    glBindTexture( GL_TEXTURE_2D, textures[1] );
    ...
    mat4 transform_cube = Translate( translation.x, translation.y, translation.z ) * Scale(scalation.x, scalation.y, scalation.z );
    
    mat4 transform_tank = transformTank * transform_cube ;
...
void drawSphere(vec4 ka, vec4 kd, vec4 ks, vec3 translation, vec3 scalation)
{
    ...
    SetMaterial( ka, kd, ks, 0.5);
    glUniform1i( glGetUniformLocation(program, "light_out"), true );
    
    mat4 transform_bube1 = Translate( translation.x, translation.y, translation.z ) *
...
display( void )
{
    ...
    //call the texture according to which picture is in the array
   /* glBindTexture( GL_TEXTURE_2D, textures[1] );
    glUniform1i( glGetUniformLocation(program, "texture_on"),false );
    
    SetMaterial( vec4(0.1, 0.1, 0.1, 1.0), vec4(0.0, 0.5, 0.1, 1.0), vec4(0.2, 0.2, 0.2, 1.0), 5.5);
    glUniform1i( glGetUniformLocation(program, "light_out"), false );
...
keyboard( unsigned char key, int x, int y )
{
    switch( key ) {
        ...
        case 'w':
            Axis = Zaxis;
            TranslationTank[Axis] += 0.5;
           // TranslationTank[Axis] = fmod(TranslationTank[Axis], 360.0);
            glutPostRedisplay();
            break;
        case 's':
            Axis = Zaxis;
